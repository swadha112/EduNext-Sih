...
function Chatbot() {
  ...
  const handleSend = () => {
    if (input.trim() && websocketRef.current) {
      ...
      // Optional timeout to handle delayed responses
      const timeout = setTimeout(() => {
        if (loading) {
          ...
          setMessages((prevMessages) => [
            ...
            {
              from: 'ai',
              message: 'Sorry, I am taking longer than usual...',
              animation: 'pop-in',
            },
          ]);
        }
      ...
      return () => clearTimeout(timeout); // Cleanup timeout if WebSocket responds
    }
  };
